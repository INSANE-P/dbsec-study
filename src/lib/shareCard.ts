...
export const SHARE_URL = "https://insane-p.github.io/dbsec-study/";
...
async function renderCard(info: ShareInfo, avatarImg: HTMLImageElement | null): Promise<Blob | null> {
  ...
  ctx.fillText("데이터베이스 보안 공부 · 성장 카드", cx, 86);
  ...
  ctx.fillText("insane-p.github.io/dbsec-study", cx, 866);
...
export async function nativeShareCard(blob: Blob, text: string): Promise<boolean> {
  ...
  try {
    await nav.share({ files: [new File([blob], "dbsec-growth.png", { type: "image/png" })], title: "데이터베이스 보안 공부", text });
    return true;
  } catch {
    return false;
  }
...
export function downloadBlob(blob: Blob, filename = "dbsec-growth.png") {
